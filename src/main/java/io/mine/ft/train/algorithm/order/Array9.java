package io.mine.ft.train.algorithm.order;
//《数据结构（Java版）（第4版）》，作者：叶核亚，2014年8月22日
//第9章   排序  9.1~9.4 排序

import io.mine.ft.train.structure.linear.random.Array1;
...
//                if (keys[j]>keys[min])                     //（降序）
	...
	// （第4版）9.3.2 堆排序
	/*
	 * public static void heapSort(int[] keys) //堆排序（降序），最小堆 { heapSort(keys,true);
	 * } //堆排序，当minheap为true时，创建最小堆，降序排序；否则创建最大堆，升序排序 public static void
	 * heapSort(int[] keys, boolean minheap) { for (int i=keys.length/2-1; i>=0;
	 * i--) //创建最小/大堆，根结点值最小/大 sift(keys, i, keys.length-1, minheap);
	 * System.out.print("最"+(minheap?"小":"大")+"堆序列："); Array1.print(keys);
	 * System.out.println("非递归算法，最小堆？ "+isHeap(keys,true)+"，最大堆？ "+isHeap(keys,false
	 * )); System.out.print("堆排序（"+(minheap?"降":"升")+"序）："); for (int
	 * i=keys.length-1; i>0; i--) //每趟将最小/大值交换到后面，再调整成最小/大堆 { swap(keys, 0, i);
	 * //交换keys[0]与keys[i] sift(keys, 0, i-1, minheap); } Array1.print(keys); }
	 * //将keys数组中以parent为根的子树调整成最小/大堆，子序列范围为parent～end。 private static void
	 * sift(int[] keys, int parent, int end, boolean minheap) { //
	 * System.out.print("sift  "+parent+".."+end+"  "); int child=2*parent+1;
	 * //child是parent的左孩子 int value=keys[parent]; while (child<=end)
	 * //沿较小/大值孩子结点向下筛选 { if (child<end && (minheap ? keys[child]>keys[child+1] :
	 * keys[child]<keys[child+1])) child++; //child记住孩子值较小/大者 if (minheap ?
	 * value>keys[child] : value<keys[child]) //若父母结点值较小/大 { keys[parent] =
	 * keys[child]; //将较小/大孩子结点值上移 parent = child; //parent、child两者都向下一层 child =
	 * 2*parent+1; } else break; } keys[parent] = value; //当前子树的原根值调整后的位置 //
	 * Array1.print(keys); }
	 */
