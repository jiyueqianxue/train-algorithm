package io.mine.ft.train.structure.linear.double_t;
//《数据结构（Java版）（第4版）》，作者：叶核亚，2014年6月23日
//2.3.3   双链表
//3. 循环双链表

//循环双链表类，实现ADT List<T>声明的方法，T表示数据元素的数据类型
public class CirDoublyList_ex<T>
{
    public DoubleNode<T> head;                             //头指针

    public CirDoublyList_ex()                                 //构造空循环双链表
    {
    ...
    public CirDoublyList_ex(CirDoublyList_ex<T> list)            	//深拷贝构造方法，复制循环双链表
    {
        this();                                          	//创建空循环双链表，只有头结点
        DoubleNode<T> rear = this.head;
...
//        throw new IndexOutOfBoundsException(i+"");         //抛出序号越界异常
	...
	public boolean equals(Object obj)
    {
        if (obj == this)
            return true;
        if (!(obj instanceof CirDoublyList_ex<?>))
            return false;
        DoubleNode<T> p=this.head.next;
        
		CirDoublyList_ex<T> list = (CirDoublyList_ex<T>)obj;
